/**
 * Vector Bench — TypeScript: one heap object per vector {x,y,z}.
 */
const N = 30_000;
...
function work(): void {
  ...
  for (let i = 0; i < N; i++) {
    const v = vecs[i];
    let x = v.x + Math.sin(phase + i * 0.07) * 0.002;
    let y = v.y + Math.cos(phase * 0.9 + i * 0.08) * 0.002;
    let z = v.z + Math.sin(phase * 0.5 + i * 0.06) * 0.002;
...
function frame(now: number): void {
  ...
  ctx.fillText(`${N.toLocaleString()}× normalize · Vec3[] · TypeScript`, 8, 18);
