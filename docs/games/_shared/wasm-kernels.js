...
(function (global) {
  ...
  const O = {
    ...
    f64_store: (a = 3, o = 0) => [0x39, ...uleb(a), ...uleb(o)],
    /** memarg align = log2(byte alignment); i32 needs align 2 (4 bytes). */
    i32_load: (a = 2, o = 0) => [0x28, ...uleb(a), ...uleb(o)],
  ...
  /** Sum of n f64 values starting at ptr (bytes). */
  function buildSumWasm() {
    return wasmModule({
      types: [funcType([I32, I32], [F64])],
      funcs: [[0]],
      mems: [[0x00, 0x01]],
      exports_: [
        ["memory", 2, 0],
  ...
  /** y[i] += a * x[i] for i in [0, len). */
  function buildSaxpyWasm() {
    return wasmModule({
  ...
  /** Dot product of two i32 arrays (length n), result i32 (wrap). */
  function buildIDotWasm() {
    return wasmModule({
      types: [funcType([I32, I32, I32], [I32])],
      funcs: [[0]],
      mems: [[0x00, 0x01]],
      exports_: [
        ["memory", 2, 0],
