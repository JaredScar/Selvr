(() => {
  ...
  function step(ts) {
    ...
    for (const o of orbs) {
      ...
      if (o.y + ORB_R >= HEIGHT - 28 && o.y <= HEIGHT - 8 && Math.abs(o.x - px) < PLAYER_W / 2 + ORB_R) {
        o.caught = true;
        score += 1;
  ...
  function draw() {
    ...
    g.fillText(`TypeScript build  \xB7  score ${score}  \xB7  misses ${missed}/8`, 12, 26);
    for (const o of orbs) {
      if (o.caught) continue;
      g.beginPath();
  ...
  function onKey(e) {
    const step2 = 14;
    if (e.key === "ArrowLeft" || e.key === "a") playerX = Math.max(0, playerX - step2);
    if (e.key === "ArrowRight" || e.key === "d")
      playerX = Math.min(WIDTH - PLAYER_W, playerX + step2);
  }
  function boot() {
